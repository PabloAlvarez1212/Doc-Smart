...
class UsuarioFactory(factory.django.DjangoModelFactory):
    ...
    fecha_nacimiento = factory.LazyFunction(lambda: fake.date_of_birth(minimum_age=18, maximum_age=80))
    estatura         = factory.LazyFunction(lambda: round(fake.pyfloat(min_value=1.50, max_value=1.95), 2))
    peso             = factory.LazyFunction(lambda: round(fake.pyfloat(min_value=45.0, max_value=120.0), 1))
    correo           = factory.LazyFunction(fake.unique.email)
    contraseña       = factory.LazyFunction(fake.password)
    cedula           = factory.LazyFunction(lambda: fake.unique.numerify('##########'))
