...
export function wrapSpecialtyLabel(value) {
    const lines = [];
    let currentLine = "";

    for (let word of String(value ?? "").trim().split(/\s+/)) {
        while (word.length > MAX_LABEL_CHARS) {
            if (currentLine) {
                lines.push(currentLine);
                currentLine = "";
            }
            lines.push(word.slice(0, MAX_LABEL_CHARS));
            word = word.slice(MAX_LABEL_CHARS);
        }
        if (!word) continue;
        if (currentLine && `${currentLine} ${word}`.length > MAX_LABEL_CHARS) {
            lines.push(currentLine);
            currentLine = word;
        } else {
...
export function SpecialtyTick({ x, y, payload }) {
...
export function getSpecialtyChartLayout(data, totalKey) {
    ...
    const maxTotal = Math.max(1, ...data.map((item) => {
        const value = Number(item[totalKey]);
